...
void dijkstraPeso(int x,int k) {
	...
	while(!pq.empty()) {
		...
		pq.pop();
		for(pii u : viz[atual.S]) {
			if(disPeso[u.S]>pii(disPeso[atual.S].S+u.F+k,disPeso[atual.S].F+1)) {
				disPeso[u.S] = {disPeso[atual.S].S+u.F+k,disPeso[atual.S].F+1};
	...
	for(int t=0;t<T;t++) {
		...
		while(l<r) {
			if(l==r) {
				...
				bool ok = 1;
				for(int i=1;i<=n;i++) {
					if(disPeso[i]!=disQuantidadeAresta[i]) {
						ok = 0;
						break;
					}
				}
